"""
    GPU Memory Usage WARNING!!! IT IS A NON-OPTIMIZED FREAKING HUGE ASS MODEL
    Created by Chenyang Huang, Feb, 2019
"""
import os
...
from tasks.Task3.hred_predictor_soft_deepmoji_soft_bert_elmo_mtl import HierarchicalPredictor, NUM_EMO
...
parser.add_argument('-loss', default='ce', type=str,
                    help="ce or focal ")
parser.add_argument('-dim', default=1200, type=int,
                    help="post name")
parser.add_argument('-ctx_att', default='True', type=str,
                    help="post name")
...
class TrainDataSet(Dataset):
    ...
    def make_bert_maks(self, bert_ids):
        if len(bert_ids) > BERT_PAD_LEN:
            bert_ids = bert_ids[:BERT_PAD_LEN]
            bert_mask = [1] * BERT_PAD_LEN
        else:
            bert_ids = bert_ids + [0] * (BERT_PAD_LEN - len(bert_ids))
            bert_mask = [1] * len(bert_ids) + [0] * (BERT_PAD_LEN - len(bert_ids))
    ...
    def read_data(self, data_list, target_list):
        ...
        for X, y in zip(data_list, target_list):
            ...
            # BERT
            bert_a = bert_tokenizer.tokenize(clean_a)
            bert_b = bert_tokenizer.tokenize(clean_b)
            bert_c = bert_tokenizer.tokenize(clean_c)

            bert_a = ['[CLS]'] + bert_a + ['[SEP]']
            bert_b = ['[CLS]'] + bert_b + ['[SEP]']
            bert_c = ['[CLS]'] + bert_c + ['[SEP]']

            bert_a = bert_tokenizer.convert_tokens_to_ids(bert_a)
            bert_b = bert_tokenizer.convert_tokens_to_ids(bert_b)
            bert_c = bert_tokenizer.convert_tokens_to_ids(bert_c)

            bert_a, bert_a_mask = self.make_bert_maks(bert_a)
            bert_b, bert_b_mask = self.make_bert_maks(bert_b)
            bert_c, bert_c_mask = self.make_bert_maks(bert_c)

            self.bert_a.append(bert_a)
            self.bert_b.append(bert_b)
            self.bert_c.append(bert_c)
            self.bert_a_mask.append(bert_a_mask)
            self.bert_b_mask.append(bert_b_mask)
            self.bert_c_mask.append(bert_c_mask)
    ...
    def __getitem__(self, idx):
        return torch.LongTensor(self.a[idx]), torch.LongTensor([self.a_len[idx]]), \
               ...
               torch.LongTensor(self.bert_a[idx]), torch.LongTensor(self.bert_a_mask[idx]), \
               torch.LongTensor(self.bert_b[idx]), torch.LongTensor(self.bert_b_mask[idx]), \
               torch.LongTensor(self.bert_c[idx]), torch.LongTensor(self.bert_c_mask[idx]), \
    ...
    def one_fold(num_fold, train_index, dev_index):
        ...
        while True:
            ...
            model = HierarchicalPredictor(SENT_EMB_DIM, SENT_HIDDEN_SIZE, num_of_vocab, USE_ELMO=True, ADD_LINEAR=False,
                                          BERT_MODEL=BERT_MODEL, USE_CTX_ATT=USE_CTX_ATT, IS_HALF=HALF_PRECISION)
            ...
            if HALF_PRECISION:
                ...
                scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer.optimizer, gamma=opt.gamma)
            else:
            ...
            for num_epoch in range(MAX_EPOCH):
                ...
                train_loss = 0
                model.train()
                for i, (a, a_len, b, b_len, c, c_len, emoji_a, emoji_b, emoji_c,
                        bert_a, bert_a_mask, bert_b, bert_b_mask, bert_c, bert_c_mask,
                        e_c, e_c_binary, e_c_emo) \
                    ...
                    pred, pred2, pred3 = model(a.cuda(), a_len, b.cuda(), b_len, c.cuda(), c_len,
                                               ...
                                               bert_a.cuda(), bert_a_mask.cuda(), bert_b.cuda(), bert_b_mask.cuda(),
                                               bert_c.cuda(), bert_c_mask.cuda())

                    if HALF_PRECISION:
                        loss_label = loss_criterion(pred, e_c.view(-1).cuda()).cuda()
                        loss_label = torch.matmul(torch.gather(weight_label.half(), 0, e_c.view(-1).cuda()),
                                                  loss_label)/e_c.view(-1).shape[0]

                        loss_binary = loss_criterion_binary(pred2, e_c_binary.view(-1).cuda()).cuda()
                        loss_binary = torch.matmul(torch.gather(weight_binary.half(), 0, e_c_binary.view(-1).cuda()),
                                                   loss_binary)/e_c.view(-1).shape[0]
                        loss_emo = loss_criterion_emo_only(pred3, e_c_emo.cuda().half())
                    else:
                        loss_label = loss_criterion(pred, e_c.view(-1).cuda()).cuda()
                    ...
                    with torch.no_grad():
                        ...
                        if HALF_PRECISION:
                            ...
                            loss_binary = torch.matmul(
                                torch.gather(weight_binary.half(), 0, e_c_binary.view(-1).cuda()),
                                loss_binary) / e_c.view(-1).shape[0]
                            loss_emo = loss_criterion_emo_only(pred3, e_c_emo.cuda().half())
                        else:
                            loss_label = loss_criterion(pred, e_c.view(-1).cuda()).cuda()
                ...
                for i, (a, a_len, b, b_len, c, c_len, emoji_a, emoji_b, emoji_c,
                        bert_a, bert_a_mask, bert_b, bert_b_mask, bert_c, bert_c_mask
                        ) in enumerate(gold_dev_data_loader):
                    with torch.no_grad():
            ...
            # print('Real Test testing')
            # pred_list_test = []
            # model.eval()
            # for i, (a, a_len, b, b_len, c, c_len, emoji_a, emoji_b, emoji_c) in enumerate(final_test_data_loader):
            #     with torch.no_grad():
            #         elmo_a = elmo_encode(a)
            #         elmo_b = elmo_encode(b)
            #         elmo_c = elmo_encode(c)
            ...
            #     del elmo_a, elmo_b, elmo_c, a, b, c, pred
            # pred_list_test = np.argmax(np.concatenate(pred_list_test, axis=0), axis=1)
            # real_test_results.append(pred_list_test)
